...
export function useKeaktifan() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        toast.error("Gagal memuat data keaktifan.");
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        toast.error("Gagal memuat nilai keaktifan.");
      } finally {
  ...
  const addTask = useCallback(async (name: string) => {
    if (!subjectId) return;
    try {
      const res = await TaskService.create({ gradeSubjectId: subjectId, name, category: "keaktifan" });
      if (res?.result) {
        setTasks((prev) => [...prev, res.result as Task].sort((a, b) => a.order - b.order));
        toast.success("Keaktifan berhasil ditambahkan");
      }
    } catch {
      toast.error("Gagal menambah keaktifan");
    }
  }, [subjectId]);
  ...
  const editTask = useCallback(async (id: string, name: string) => {
    try {
      ...
      if (res?.result) {
        ...
        toast.success("Keaktifan berhasil diubah");
      }
    } catch {
      toast.error("Gagal mengubah keaktifan");
    }
  }, []);
  ...
  const removeTask = useCallback(async (id: string) => {
    try {
      ...
      toast.success("Keaktifan berhasil dihapus");
    } catch {
      toast.error("Gagal menghapus keaktifan");
    }
  }, [selectedTaskId]);
