...
export function usePenilaian(category: string) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await TaskService.getAll(subjectId, category);
        const allTasks = res?.result || [];
        setTasks(allTasks.filter((t: any) => category === "tugas" ? (t.category === "tugas" || !t.category) : t.category === category));
  ...
  const addTask = useCallback(async (name: string) => {
    if (!subjectId) return;
    try {
      const res = await TaskService.create({ gradeSubjectId: subjectId, name, category });
      if (res?.result) {
