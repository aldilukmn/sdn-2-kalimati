...
interface TabPartisipasiProps {
  paginatedStudents: Array<{ studentId: string; name: string }>;
  partisipasiScores: Record<string, number | undefined>;
  partisipasiLoading: boolean;
...
export default function TabPartisipasi({
  paginatedStudents,
  partisipasiScores,
  partisipasiLoading,
  ITEMS_PER_PAGE,
  ...
  const headers = (
    <>
      ...
      <th className="text-center px-4 py-3 font-semibold whitespace-nowrap w-32">Rerata Nilai Partisipasi</th>
  ...
  return (
    ...
    >
      {paginatedStudents.map((s, i) => {
        const scoreValue = partisipasiScores[s.studentId];
