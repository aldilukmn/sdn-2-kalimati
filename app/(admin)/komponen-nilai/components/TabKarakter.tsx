...
interface TabKarakterProps {
  ...
  karakterLoading: boolean;
  ITEMS_PER_PAGE: number;
  currentPage: number;
  karakterTotalPages: number;
  setCurrentPage: (page: number) => void;
  totalKarakterStudents: number;
}

export default function TabKarakter({
  paginatedKarakterStudents,
  karakterLoading,
  ITEMS_PER_PAGE,
  currentPage,
  karakterTotalPages,
  setCurrentPage,
  totalKarakterStudents,
}: TabKarakterProps) {
  ...
  const headers = (
    <>
      ...
      <th className="text-center px-4 py-3 font-semibold whitespace-nowrap w-40">Rata-rata Nilai Karakter</th>
  ...
  return (
    <ScoreTableWrapper
      loading={karakterLoading}
      empty={paginatedKarakterStudents.length === 0}
      emptyMessage="Belum ada data penilaian karakter untuk kelas ini."
      totalPages={karakterTotalPages}
      totalItems={totalKarakterStudents}
