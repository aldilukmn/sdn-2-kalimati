...
interface TabKeaktifanProps {
  paginatedStudents: Array<{ studentId: string; name: string }>;
  keaktifanScores: Record<string, number | undefined>;
  keaktifanLoading: boolean;
...
export default function TabKeaktifan({
  paginatedStudents,
  keaktifanScores,
  keaktifanLoading,
  ITEMS_PER_PAGE,
  ...
  const headers = (
    <>
      ...
      <th className="text-center px-4 py-3 font-semibold whitespace-nowrap w-32">Rerata Nilai Keaktifan</th>
  ...
  return (
    ...
    >
      {paginatedStudents.map((s, i) => {
        const scoreValue = keaktifanScores[s.studentId];
