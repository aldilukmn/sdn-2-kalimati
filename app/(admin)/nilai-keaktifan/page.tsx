...
import { useKeaktifan } from "@/hooks/useKeaktifan";
import { ITEMS_PER_PAGE } from "@/lib/constants";

export default function NilaiKeaktifanPage() {
  const {
    role,
    semester,
    setSemester,
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <PageHero
        ...
        description="Kelola keaktifan dan input nilai"
      />
      ...
      {error ? (
        ...
        <div className="bg-white/70 dark:bg-gray-800/40 border border-white/20 dark:border-gray-700/50 shadow-lg rounded-2xl p-8 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Pilih mata pelajaran untuk mulai mengelola keaktifan
          </p>
        </div>
      ) : loading ? (
        <LoadingSkeleton rows={1} />
        ...
        <>
          ...
          <div className="space-y-2">
            {tasks.length === 0 ? (
              <EmptyState
                icon={ClipboardList}
                title="Belum ada keaktifan"
                description="Klik Tambah Keaktifan untuk membuat keaktifan baru"
              />
            ) : (
              tasks.map((t, index) => {
          ...
          {selectedTask && (
            <div className="bg-white/70 dark:bg-gray-800/40 border border-white/20 dark:border-gray-700/50 shadow-lg rounded-2xl p-4 md:p-5">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-semibold text-slate-700 dark:text-slate-200">
                  Input Nilai Keaktifan: {selectedTask.name}
                </h2>
              </div>
      ...
      <Modal
        ...
        title={taskModal?.mode === "add" ? "Tambah Keaktifan" : "Edit Keaktifan"}
      >
        <div className="space-y-3">
          <input
            ...
            placeholder="Masukkan nama keaktifan"
      ...
      <Modal
        ...
        title="Hapus Keaktifan"
      >
        <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
          Yakin ingin menghapus keaktifan ini? Semua nilai terkait juga akan
          terhapus.
        </p>
