...
export default function LogoutButton() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {dropdownOpen && (
        <div className="absolute right-0 mt-2 w-48 p-1 flex flex-col gap-0.5 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 z-50 transition-all duration-100 opacity-100 translate-y-0">
          <button
            onClick={() => { setDropdownOpen(false); router.push("/profil"); }}
            className="w-full flex items-center gap-3 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors cursor-pointer"
          >
            <User size={16} />
            Profil
          </button>
          <div className="h-px bg-slate-200 dark:bg-slate-700 my-0.5 mx-2" />
          <button
            onClick={handleLogout}
            disabled={isLoading}
            className="w-full flex items-center gap-3 px-3 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
